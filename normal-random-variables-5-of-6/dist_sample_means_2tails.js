$(document).ready(function(){
	...
	var zbox = document.getElementById("zbox").value;
	var zbox2 = document.getElementById("zbox2").value;
    ...
    var graph = svg.append("g").attr("class", "curve"),
    	flags = svg.append("g").attr("class", "flags"),
       	...
       	axislabel = svg.append("text").text("Z-score").attr("x",(width)/2-30)
    ...
    update(graph, axis, mean, SD);
   	...
   	function update(graph, axis, mean, SD){
		...
		renderCurve(graph, data, x, y, binWidth, "darkblue", "curve");
	...
	function addFlags(graph, data, xScale, yScale, mean, SD, binWidth){
		...
		addFunctionality(lFlag, data, flags, xScale, yScale, mean, SD, "left", binWidth, rFlag);
		addFunctionality(rFlag, data, flags, xScale, yScale, mean, SD, "right", binWidth, lFlag);

	}

	function addFunctionality(flag, data, graph, xScale, yScale, mean, SD, side, binWidth, flag2){
		var max = mean + 4 * SD,
			min = mean - 4 * SD;
		...
		var drag = d3.behavior.drag();
		 ...
		 enter.on("click",function(){
  				...
  				var change = 250+zbox*460/8;
				move2(flag2, change, minX, maxX);
				updatePositionText2(flag2, zbox);
	            updateSegments(graph, data, xScale, yScale, mean, SD, min, max, binWidth);

  				zbox2 = document.getElementById("zbox2").value;
  				var change = 250+zbox2*460/8;
				move2(flag, change, minX, maxX);
				updatePositionText2(flag, zbox2);
				updateSegments(graph, data, xScale, yScale, mean, SD, min, max, binWidth);
    ...
    function updateSegments(graph, data, xScale, yScale, mean, SD, min, max, binWidth){
    	...
    	var rightIndex = Math.floor((right-min)*hash);
    	...
    	if(left < right){
    		...
    		showPercentile(leftFlag, left, right, "left", mean, SD, min, max);
    		//showPercentile(rightFlag, right, "right", mean, SD, min, max);
    	}else{
    		var arr = data.slice(rightIndex,leftIndex);
    		renderCurve(graph, arr, xScale, yScale, binWidth, fadeColor, "faded");
    ...
    function showPercentile(flag, left, right, side, mean, SD, min, max){
    	if(right > left){
    		var realright = right.toPrecision(4);
    		var realleft = left.toPrecision(4);
    	}
    	else{
    		var realright = left.toPrecision(4);
    		var realleft = right.toPrecision(4);
    	}
    	d3.select("#lefttext").text("The left tail has area: " + (cdf(realleft, mean, SD)).toFixed([4]));
		d3.select("#middle").text("The central section has area: " + (cdf(realright, mean, SD)-cdf(realleft, mean, SD)).toFixed([4]));
		d3.select("#righttext").text("The right tail has area: " + (1-cdf(realright, mean, SD)).toFixed([4]));;
    }
	...
	function updatePositionText(elem, xScale){
		var scaledX = getX(elem, xScale).toPrecision(3);
		elem.select("text.x").text("Z-score = " + scaledX);
	}

	function updatePositionText2(elem, xScale){
		//var scaledX = getX(elem, xScale).toPrecision(4);
		elem.select("text.x").text("Z-score: " + xScale);
	}
	...
	function renderFlag(g, side){
		var flagWidth = 5;
		var color = ((side == "left") ? "darkorange" : "darkorange");
