...
dotplot= function(){
	...
	var labels = {
		values: 	{x: (width), y: 0, anchor: "end", offset: 10},
	...
	function d3_dotplot_histogramize(plot){
		var data = plot.data, histogram = [];
		//alert("data: " + data);
		for(var i = 0; i < (20); i++){
			//alert("max and min: " + plot.max + " " + plot.min)
			histogram.push(0);
		}
		for(var i = 0, x = data.length; i < x; i++){
			histogram[Math.round((data[i] - plot.min)*20)]++;
		}
	...
	function d3_dotplot_ADM(plot){
		...
		if(ADM > 0){
			...
			d3_dotplot_ln(lines, plot.x2(minusADM), plot.x2(mean), admh, admh, ADM.toString());
			d3_dotplot_ln(lines, plot.x2(mean), plot.x2(plusADM), admh, admh, ADM.toString());
